...
$$
\begin{equation}
\text{logit}(\pi_{ij,k}) = \alpha_k + (u_{i,k} + \beta_M M_i) + v_{j,k}
\end{equation}
$$
...
for(k in 1:K) {
  ...
  for(i in 1:S) for(j in 1:S) {
    pi_ijk = inv_logit(alpha_k[k] + u_i[i] + v_j[j])
    A_true[i,j] = rbinom(1, 1, pi_ijk)
    A_obs[i,j] = rbinom(1, 1, p_k[k] * pi_ijk)
    
    # body mass model 
    .pi_ijk = inv_logit(alpha_k[k] + (u_i[i] + bM * M[i]) + v_j[j])
    A_Mtrue[i,j] = rbinom(1, 1, .pi_ijk)
    A_Mobs[i,j] = rbinom(1, 1, p_k[k] * .pi_ijk)
