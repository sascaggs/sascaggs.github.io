...
Food webs are often represented as network graphs, where nodes represent species and links represent observed feeding interactions, or potential feeding interaction inferred from species traits. The [Web of Life](https://www.web-of-life.es/) database houses many such graphs, showcasing their interesting structural patterns and the tremendous efforts ecologists have gone to in order to collect these challenging data.  
...
We assume that each web has its own detection probability $\phi_k$. The parameter $\mu_k$ is the average detection probability across all food webs, and $\delta_{p,k}$ is a specific deviation of web $k$ from the global average, which we model as non-centered: $\delta_{\phi,k} \sim \mathcal{N}(0,\sigma^2_{\phi})$. 
