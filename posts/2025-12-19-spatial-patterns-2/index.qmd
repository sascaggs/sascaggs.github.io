...
the state configuration $P(s)$ depends on a Hamiltonian energy $H(s)$. This energy is based on user-defined local neighborhood interactions

$$
H(s) = -J\sum_{\langle i, j\rangle} \delta(s_i,s_j)
$$ {#eq-ising-hamiltonian}

where $\langle i, j \rangle$ identifies the neighboring sites. The Kronecker $\delta$ returns a 1 when the states of two neighbors are the same ($s_i = s_j$), and a 0 when they are not ($s_i \ne s_j$). For the Ising Model, $s_i \in \{-1,+1\}$ which can then be written simply has $s_i s_j$. For the Potts model, $s_i \in \{1,...,q\}$ with $q$ being the number of discrete states.

The parameter $\beta$ controls the degree of stochasticity in the energetic alignment (e.g., inverse temperature), and the parameter $J$ determines the magnitude and direction of the alignment between neighboring states.
...
and identify the neighbors of $(i,j)$. In this case, we use the von Neumann neighborhood. 
