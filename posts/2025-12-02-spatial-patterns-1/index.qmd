...
```
```{r, echo=F, fig.width=3.618, fig.height=3, dpi=300, units = 'in'}
...
```
```{r, echo=F, fig.width=4, fig.height=4, dpi=300}
df = bind_rows(X_list)
...
cp = bind_rows(C_list) |> 
  ...
  geom_line(size=1) + 
  scale_color_manual(values = c("#CD00CD", "#8844CD", "#4488CD", "#00CDCD")) + 
  labs(color = '\u2113', 
       x = 'd', y = TeX("$\\hat{C}(d)$"))   
